...
class FeatureContext implements Context, SnippetAcceptingContext {
	...
	private function assertRooms($rooms, TableNode $formData) {
		...
		Assert::assertEquals($formData->getHash(), array_map(function ($room, $expectedRoom) {
			if (isset($room['participants']) && isset($expectedRoom['participants'])) {
	...
	/**
	 * @When /^user "([^"]*)" sets description for room "([^"]*)" to "([^"]*)" with (\d+)$/
	 *
	 * @param string $user
	 * @param string $identifier
	 ...
	 * @param TableNode
	 */
	public function userSetsDescriptionForRoomTo($user, $identifier, $description, $statusCode) {
		$this->setCurrentUser($user);
		$this->sendRequest(
			'PUT', '/apps/spreed/api/v3/room/' . self::$identifierToToken[$identifier] . '/description',
